    ...
    fun `자동 설정은 transfer 관련 빈을 등록한다`() {
        val context = AnnotationConfigApplicationContext().apply {
            beanFactory.registerSingleton(
                ...
                mock(TransferAccountClient::class.java),
            )
            beanFactory.registerSingleton(
                "transferRepository",
                mock(TransferRepository::class.java),
            )
            beanFactory.registerSingleton(
            ...
            beanFactory.registerSingleton(
                ...
                mock(DailyLimitUsageRepository::class.java),
            )
            beanFactory.registerSingleton(
                "transferMemberClient",
                mock(TransferMemberClient::class.java),
            )
            beanFactory.registerSingleton(
                "transferSnapshotUtil",
                mock(TransferSnapshotUtil::class.java),
            )
            beanFactory.registerSingleton("ledgerRepository", mock(LedgerRepository::class.java))
            beanFactory.registerSingleton(
                "transferEventPublisher",
                mock(TransferEventPublisher::class.java),
            )
            beanFactory.registerSingleton(
                "clock",
        ...
        assertThat(context.getBean(TransferQueryUseCase::class.java)).isInstanceOf(
            TransferQueryService::class.java,
        )
        assertThat(context.getBean(TransferCreateUseCase::class.java)).isInstanceOf(TransferService::class.java)
        assertThat(context.getBean(TransferEventPublishUseCase::class.java)).isInstanceOf(
            TransferEventPublishService::class.java,
        )
