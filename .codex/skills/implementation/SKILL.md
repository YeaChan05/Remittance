...
1. `rg`로 가장 가까운 기존 구현 1~3개를 찾는다.
2. package, 파일 배치, 클래스 이름, 생성자 형태를 그대로 복제한다.
3. 필요한 코드만 수정한다.
4. bean 노출이 바뀌면 해당 AutoConfiguration/BeanRegistrar wiring도 같이 수정한다.
5. 검증은 관련 skill 또는 호출자 지시에 따른다.
