...
`application`, `aggregate` 같은 실행 모듈은 port와 adapter를 연결해주는 조립 지점이다
...
현재 프로젝트의 `aggregate`는 이 점을 보여주는 예시가 된다
...
이렇게 하면 `aggregate` 같은 same-process runtime에서는 HTTP 호출 없이도 `{Provider}InternalApi` 구현체가 바로 조립될 수 있다
...
`application`, `aggregate`는 assembly 역할을 맡는다
